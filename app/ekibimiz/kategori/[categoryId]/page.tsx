...
import { getTeamCategoryById, getTeamMembersByCategory } from "@/app/actions/team";
...
export default async function CategoryMembersPage({ params }: PageProps) {
  const { categoryId } = await params;

  const [categoryRes, membersRes] = await Promise.all([
    getTeamCategoryById(categoryId),
    getTeamMembersByCategory(categoryId),
  ]);

  if (!categoryRes.ok || !categoryRes.category) {
    notFound();
  }
    ...
    <div className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-10">
        ...
        <p className="text-muted-foreground">
          Gruptaki üyelere tıklayarak detaylarını görebilirsiniz.
        </p>
      </div>
      ...
      {members.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-border bg-card/30 py-16 text-center">
          ...
          <p className="text-muted-foreground">Bu kategoride henüz üye yok.</p>
        </div>
      ) : (
